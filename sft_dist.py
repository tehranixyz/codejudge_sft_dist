...
#     template = """You are an expert in code translation between {src_lang} and {tgt_lang}.
#     Below is the source code written in {src_lang}:

#     ```
#     {code}
#     ```

#     Your task is to translate this {src_lang} code into {tgt_lang}.
#     Return only the translated {tgt_lang} code, and include the comment |End-of-Code| at the end.
#     """

#     for src, tgt, code, target_code in zip(batch['source_language'], batch['target_language'], batch['source_code'], batch['target_code']):
#         prompts.append(template.format(src_lang=LANGUAGE_CONVENTIONS[src], tgt_lang=LANGUAGE_CONVENTIONS[tgt], code=code))
#         responses.append("```\n{}\n```\n|End-of-Code|".format(target_code))

#     return {'prompt': prompts, 'completion': responses}
